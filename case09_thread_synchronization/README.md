...
NOTE: In case this is modified in the future, consider using `std::hardware_destructive_interference_size` (Available from C++17) as a parameter to alignas(X), it is a global variable that returns the exact size of a cache line (normally it is 64 bytes in L1, but it could vary in the future).
